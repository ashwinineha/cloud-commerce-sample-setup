...
import com.sap.cxservices.rediscart.persistence.polyglot.repository.documentcart.Entity.EntityBuilder;
	...
	protected static class DocumentSerializer extends StdSerializer<DocumentWrapper>
	{
				...
				throws IOException
		{
			...
			catch (final WrappedIOException e)
			{
				if(LOG.isDebugEnabled()){
					LOG.debug("Error occurred while serializing.", e);
				}
				throw e.unwrap();
			}
			gen.writeEndArray();
		...
		protected void writeEntityField(final Object value, final JsonGenerator gen) throws IOException
		{
			...
			else if (value instanceof Float)
			{
				...
				writeNonPrimitive(value, gen);
			}
			gen.writeEndObject();
		}

		private void writeNonPrimitive(Object value, JsonGenerator gen) throws IOException {
		...
		private void writeMap(Map<?,?> map, JsonGenerator gen) throws IOException
		{
			...
			for (final Map.Entry entry : map.entrySet())
			{
				...
				writeEntityField(entry.getKey(), gen);
				gen.writeFieldName(VALUE);
				writeEntityField(entry.getValue(), gen);
				gen.writeEndObject();
			}
	...
	protected static class DocumentDeserializer extends StdDeserializer<Document>
	{
		...
		protected Object readEntityField(final JsonNode valueNode)
		{
			...
			switch (valueType)
			{
				...
				default:
					return readNonPrimitiveField(valueType, value);
			}
		}

		protected Object readNonPrimitiveField(final String valueType, JsonNode value){
			...
			{
				return readSetField((ArrayNode) value);
			}

			if (MAP.equals(valueType))
			{
				return readMapField((ArrayNode) value);
			}
			switch (valueType)
			{
				case REFERENCE:
		...
		private Object readMapField(ArrayNode array) {
			if (array.size() == 0)
			{
			...
			for(int i=0;i<array.size();i++){
				...
				map.put(readEntityField(node.get(KEY)), readEntityField(node.get(VALUE)));
			}
			return map;
		}
		...
		private Object readSetField(final ArrayNode array)
		{
			if (array.size() == 0)
			{
			...
			return StreamSupport.stream(Spliterators.spliterator(array.elements(), array.size(), 0), false)
					.map(this::readEntityField).collect(Collectors.toSet());
		}
